...
1. Choose a drill from the available list: `todo-list`, `autocomplete`, `sortable-table`, `pagination`, `timer`, `modal`, `grid-toggle`, `tabs`, `accordion`, `fetch-demo`.
2. Run `./swap-drill.sh <drill-name>` from the repo root; e.g. `./swap-drill.sh pagination`.
3. The script copies `main.tsx`, `App.tsx`, and `index.css` (if present) from that drill folder into `src/`, replacing what was there.
4. Run `npm run dev` (or your preferred Vite command) to start the dev server with the newly swapped drill.
